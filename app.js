...
function pullSettings(){
  ...
  // 时间：只从开关读，不覆盖 storyStart（已由 change 事件单独处理）
  S.chatOpt.timeMode=$('timeModeOn').checked?'real':'fiction';
  // 注意：storyStart 和 timeDrift 已由各自的 change 事件实时写入，pullSettings 不重复覆盖
  // 但为保险起见同步一次 drift
  S.chatOpt.timeDriftMin=+($('timeDrift').value)||0;
  S.chatOpt.storyStart=$('storyStart').value||'';
...
function collectInjections(){
  ...
  if(S.chatOpt.recallSee){
    put('depth',0,'【撤回可见】当对话中出现撤回提示时（如「某人撤回了一条消息」），你能感知到撤回这件事，但不知道具体内容，可以自然地回应或询问。',8);
  }else{
    put('depth',0,'【撤回不可见】对话中的撤回提示对你完全不可见，不要提及或询问任何关于撤回的内容，当作没有发生过。',8);
  }
  // ── AI 表情表态 ──
...
$('puNarrate')&&($('puNarrate').onclick=()=>{
  if(!hasRole()){toast('先选个角色',true);return}
  closePlus();
  const txt=prompt('旁白内容（以居中小字显示在聊天中，无头像）：','');
...
// 以角色身份发送（直接以 AI 气泡显示，我来扮演角色说话，不触发 AI 回复）
$('puRoleMode')&&($('puRoleMode').onclick=()=>{
  if(!hasRole()){toast('先选个角色',true);return}
  closePlus();
  const r=curRole();
  // 弹出输入框让用户输入角色要说的话
  const txt=prompt('以「'+r.roleName+'」的身份说（直接作为角色气泡出现，不会触发 AI）：','');
  if(txt==null||!txt.trim())return;
  // 直接 push 为 assistant 消息
  chat().push({role:'assistant',content:txt.trim(),t:Date.now(),_rolePlay:true});
  save();renderThread();
  // 不触发 AI，不显示回复按钮，静默
});
