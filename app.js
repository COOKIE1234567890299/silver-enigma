...
const DB={get(k){if(storeOK){try{return localStorage.getItem(k)}catch(e){}}return mem[k]??null},set(k,v){if(storeOK){try{localStorage.setItem(k,v);return}catch(e){}}mem[k]=v}};
...
function freshState(){return{
  apiPresets:[{name:'默认',provider:'claude',baseUrl:'',model:defModels.claude,apiKey:''}],apiIdx:0,
  roleCards:[],roleIdx:0,
  ...
  worldBook:[],
  voicePresets:[{name:'默认',engine:'elevenlabs',base:'',key:'',voice:'',model:'eleven_v3',autoSpeak:false,showRaw:false}],voiceIdx:0,
  ...
  chatOpt:{split:true,splitInject:'depth',splitDepth:0,trans:false,transInject:'depth',transDepth:0,autoReply:true,showTime:true,fontSize:15,typingDelay:false,typingDelaySec:3,readNoReply:false,charStickers:true,charKaomoji:true,autoStatus:true,timeMode:'real'},
...
function setBg(el,src){el.style.backgroundImage=src?`url(${src})`:'';if(src)el.textContent=''}
...
// 把世界书和正则合并到 rules 里咯
const TAB_GROUPS={conn:['model','voice'], play:['inject','chat','mind','jailbreak','aireply'], rules:['world','regex','appearance'], memory:['memory'], adv:['debug','data']};
...
function syncSettings(){
  ...
  $('timeModeOn').checked=(S.chatOpt.timeMode||'real')!=='fiction';
...
function syncVoice(){const v=curVoice();$('vEngine').value=v.engine;$('vBase').value=v.base||'';$('vKey').value=v.key||'';$('vVoice').value=v.voice||'';$('vModel').value=v.model||'';$('autoSpeak').checked=!!v.autoSpeak;$('showRaw').checked=!!v.showRaw;updVoiceUI()}
...
function pullSettings(){
  ...
  const v=curVoice();v.engine=$('vEngine').value;v.base=$('vBase').value;v.key=$('vKey').value;v.voice=$('vVoice').value;v.model=$('vModel').value;v.autoSpeak=$('autoSpeak').checked;v.showRaw=$('showRaw').checked;
  ...
  S.chatOpt.timeMode=$('timeModeOn').checked?'real':'fiction';
...
const vOps=opsFor(S.voicePresets,()=>S.voiceIdx,i=>S.voiceIdx=i,{engine:'elevenlabs',base:'',key:'',voice:'',model:'eleven_v3',autoSpeak:false,showRaw:false});
...
function renderRules(){
  ...
  if(!reg.rules||!reg.rules.length){box.innerHTML='<div class="wb-empty">还没有规则。点「新建分组」或「添加规则」。</div>';return}
  // 修正旧字符串格式
  ...
  function renderRxGroup(groupName,items){
    ...
    gbtn.onclick=e=>{e.stopPropagation();openMsgSheet('分组：'+groupName,[
      {label:'改名',fn:()=>{const n=prompt('新分组名：',groupName);if(!n||!n.trim())return;items.forEach(({r})=>r.group=n.trim());save();renderRules();toast('已改名')}},
      {label:'添加规则到此分组',fn:()=>{const newR={find:'',replace:'',on:true,target:'both',name:'新规则',group:groupName,_isNew:true};reg.rules.push(newR);save();renderRules();openRxEdit(reg.rules.length-1)}},
      {label:'删除整个分组（含规则）',danger:true,fn:()=>{if(!confirm('删除分组「'+groupName+'」及其全部 '+items.length+' 条规则？'))return;const idxs=new Set(items.map(x=>x.i));reg.rules=reg.rules.filter((_,i2)=>!idxs.has(i2));save();renderRules();toast('已删除分组')}},
    ])}
...
$('addRxGroup').onclick=()=>{const n=prompt('新分组名称：','');if(!n||!n.trim())return;const newR={find:'',replace:'',on:true,target:'both',name:'新规则',group:n.trim(),_isNew:true};curRegex().rules.push(newR);save();renderRules();openRxEdit(curRegex().rules.length-1)};
$('addRule').onclick=()=>{
  ...
  reg.rules.push({find:'',replace:'',on:true,target:'both',name:'新规则',group,_isNew:true});save();renderRules();openRxEdit(reg.rules.length-1);
};

// 正则编辑弹窗
let rxEditIdx=-1;
function openRxEdit(i){rxEditIdx=i;const r=curRegex().rules[i];$('rxName').value=r.name||'';$('rxGroup').value=r.group||'';$('rxFind').value=r.find||'';$('rxReplace').value=r.replace||'';$('rxTarget').value=r.target||'both';$('rxScope').value=r.applyScope||'chat';$('rxOn').checked=r.on!==false;$('rxEditScrim').classList.add('show');$('rxEditModal').classList.add('show')}
...
function renderWbList(){
  ...
  if(!list.length){box.innerHTML='<div class="wb-empty">这里还没有条目。<br>点上方「新建大类」或「新建条目」。</div>';return}
  const groups={};const ungrouped=[];
  list.forEach(w=>{
...
function showGroupMenu(groupName,items,anchor){
  const actions=[
    {label:'改名',fn:()=>{const n=prompt('大类名称：',groupName);if(!n||!n.trim())return;(S.worldBook||[]).forEach(w=>{if(w.sourceGroup===groupName)w.sourceGroup=n.trim()});save();renderWbList();toast('已改名')}},
    ...
    {label:'删除整个大类（含条目）',danger:true,fn:()=>{if(!confirm('删除大类「'+groupName+'」及其全部 '+items.length+' 条条目？'))return;const ids=new Set(items.map(w=>w.id));S.worldBook=(S.worldBook||[]).filter(w=>!ids.has(w.id));(S.roleCards||[]).forEach(r=>{if(r.wbIds)r.wbIds=r.wbIds.filter(id=>!ids.has(id))});save();renderWbList();toast('已删除大类')}},
  ];
...
$('wbAddGroup').onclick=()=>{
  ...
  const w=newWB({scope:wbTab==='role'?'role':'global',name:'新条目'});
  w._isNew=true;w.sourceGroup=n.trim();
  if(wbTab==='role')w.roleId=curRole().id;
...
function startKeepAlive(){if(silentAudio)return;try{const AC=window.AudioContext||window.webkitAudioContext;const ac=new AC();const osc=ac.createOscillator();const g=ac.createGain();g.gain.value=0.0001;osc.connect(g);g.connect(ac.destination);osc.start();silentAudio={ac,osc};if('mediaSession'in navigator){navigator.mediaSession.metadata=new MediaMetadata({title:'语音角色',artist:'后台保活'})}}catch(e){}if(typeof refreshKeepStat==='function')refreshKeepStat()}
function stopKeepAlive(){if(silentAudio){try{silentAudio.osc.stop();silentAudio.ac.close()}catch(e){}silentAudio=null}if(typeof refreshKeepStat==='function')refreshKeepStat()}
...
function renderMemModalList(){
  ...
  rm.memories.forEach((m,i)=>{
    ...
    del.onclick=()=>{if(!confirm('删除这条记忆？'))return;rm.memories.splice(i,1);save();renderMemModalList()};
...
$('fileImport').onchange=e=>{const f=e.target.files[0];if(!f)return;const rd=new FileReader();rd.onload=()=>{try{S=JSON.parse(rd.result);const fr=freshState();for(const k in fr)if(S[k]==null)S[k]=fr[k];
  ...
  (S.regexPresets||[]).forEach(p=>{if(Array.isArray(p.rules))p.rules=p.rules.map(rr=>typeof rr==='string'?{find:rr,replace:'',on:true,target:'both',name:rr.slice(0,12)||'规则',group:''}:rr)});
...
async function doSummarize(silent){
  const rm=roleMem();if(!rm)return;const c=chat().filter(m=>!m.hidden);const from=rm.sumDone||0;const seg=c.slice(from);if(!seg.length){if(!silent)toast('没有新对话可总结',true);return}
  if(!silent)toast('总结中…');
...
function storyEpoch(){const c=curConvo();if(!c)return Date.now();if(c.clock==null)c.clock=Date.now();return c.clock}
...
// 在收到 AI 回复时应用本轮时间推进（仅虚构时间线），返回清理掉推进标记后的 mind 文本
function applyTimeAdvance(mindTxt){
  if((S.chatOpt&&S.chatOpt.timeMode)!=='fiction')return mindTxt;
  const c=curConvo();if(!c)return mindTxt;
  if(c.clock==null)c.clock=Date.now();
  const drift=((S.chatOpt&&S.chatOpt.timeDriftMin!=null)?S.chatOpt.timeDriftMin:2)*60000;
  c.clock+=drift;
  const adv=parseTimeAdvance(mindTxt);
  if(adv>0)c.clock+=adv;
  return String(mindTxt||'').split('\n').filter(l=>!/时间推进\s*[：:]/.test(l)).join('\n').trim();
}
...
function collectInjections(){
  ...
  const put=(pos,depth,txt,order)=>{if(!pos||pos==='off'||!txt||!txt.trim())return;const item={txt:txt.trim(),order:(order!=null?order:50)};if(pos==='depth'){const d=depth||0;(buckets.depthMap[d]=buckets.depthMap[d]||[]).push(item)}else buckets[pos].push(item)};
  ...
  if(S.mind && S.mind.on){
    ...
    if(md.genTime){
      if((S.chatOpt&&S.chatOpt.timeMode)==='fiction') gen.push('时间：直接照抄系统在【当前故事时间】里给你的时间，一个字都不要改、不要自己推算；时间的推进只通过下面说明的「时间推进」标记进行。');
      else gen.push('时间：直接照抄系统给你的【当前时间】，不要自己改写。');
    }
    if(gen.length){
      let text;
  ...
  // ── 当前时间 / 时间模式（权威时间引擎：时间由系统持有，AI 只读不算）──
  if((S.chatOpt&&S.chatOpt.timeMode)==='fiction'){
    put('depth',0,'【当前故事时间】现在是 '+fmtStoryTime(storyEpoch())+'。\n这是系统记录的【权威时间】，本轮请直接采用，绝对不要自己重新推算或改写时间数字。\n仅当本轮剧情出现明显时间跳跃时（如「过了三天」「一个月后」「第二天早上」「睡了一觉醒来」），在 <mind> 里【单独一行】输出：时间推进：+N单位（单位只能是 分钟/小时/天/周/月/年，例：时间推进：+3天）。系统会据此推进权威时间，并在下一轮把新时间发给你；没有跳跃就不要输出这一行，更不要直接改时间数字。',1);
  }else{
    const now=new Date();
    const wd=['周日','周一','周二','周三','周四','周五','周六'][now.getDay()];
    const dstr=now.getFullYear()+'年'+(now.getMonth()+1)+'月'+now.getDate()+'日 '+wd+' '+('0'+now.getHours()).slice(-2)+':'+('0'+now.getMinutes()).slice(-2);
    put('depth',0,'【当前时间】现在是 '+dstr+'（系统真实时间，权威，请直接采用、不要自行改写）。请结合这个真实时间来对话（问候、作息、节日等）。',1);
  }
...
function buildSystem(){const b=collectInjections();return [...b.head,...b.tail].join('\n\n')}
function buildDepthNote(){const b=collectInjections();const all=[];Object.keys(b.depthMap).sort((x,y)=>y-x).forEach(d=>all.push(...b.depthMap[d]));return all.join('\n\n')}
function carriedMessages(){const c=chat().filter(m=>!m.hidden);const n=S.memOpt.carry||20;return n>0?c.slice(-n):c.slice()}
...
function buildMessages(){
  const msgs=carriedMessages().map(m=>{
...
function pushAIReply(raw){
  ...
  let cleanRaw=raw.replace(/<mind>[\s\S]*$/i,'').trim();
  ...
  const c=chat();const parts=splitRawIntoMessages(cleanRaw);const base=Date.now();
...
function renderStickerPicker(){const g=$('stGrid');g.innerHTML='';const list=allStickers();if(!list.length){g.innerHTML='<div class="note" style="grid-column:1/-1">还没有表情，点「管理」上传。</div>';return}list.forEach(s=>{const c=document.createElement('div');c.className='st-cell';c.innerHTML=`<div class="stbox"><img src="${s.url}" alt=""></div><div class="nm">${s.name||''}</div>`;c.onclick=()=>{chat().push({role:'user',content:'{{sticker:'+s.name+'}}',t:Date.now()});save();closeStickerPicker();renderThread();if(S.chatOpt.autoReply!==false)runAI();else showReplyBtn()};g.append(c)});squareStickers(g)}
...
const voiceCache={};
function isSpecialPart(p){return /^\{\{(voice|card|img|transfer|location|gift):/.test(p.trim())||/\u0003PAT:/.test(p)}
...
function renderSpecialPart(col,p,side){
  let t=p.trim();let m;
  // 拍一拍提示（居中系统消息）
  const patM=p.match(/\u0003PAT:([^\u0003]+)\u0003/);
  if(patM){
    const r=curRole();const who=side==='ai'?(r?r.roleName||'对方':'对方'):(curUser().userName||'我');
    const div=document.createElement('div');div.className='pat-tip';
    div.textContent=who+' '+patM[1].trim();
  ...
  if((v=grab('transfer'))!=null){const segs=v.split('|');const amt=(segs[0]||'').trim();const note=(segs[1]||'').trim();const r=curRole(),u=curUser();const toName=side==='user'?(r?r.roleName||'对方':'对方'):(u.userName||'我');
    ...
    card.innerHTML='<div class="wt-top"><div class="wt-ic"><svg viewBox="0 0 24 24"><path d="M17 7L7 17M7 7h10v10"/></svg></div><div><div class="wt-amt">¥ '+(amt||'0')+'</div><div class="wt-to">转账给 '+toName+'</div></div><div class="wt-tag">转账</div></div>'+(note?'<div class="wt-note">'+note.replace(/</g,'&lt;')+'</div>':'')+'<div class="wt-foot"><span>等待对方收款</span><span>›</span></div>';
  ...
  if((v=grab('gift'))!=null){const segs=v.split('|');const name=(segs[0]||'').trim();const note=(segs[1]||'').trim();
    ...
    card.innerHTML='<div class="gbox">🎁</div>'+(name?'<div class="gname">'+name.replace(/</g,'&lt;')+'</div>':'')+(note?'<div class="gname" style="opacity:.8">'+note.replace(/</g,'&lt;')+'</div>':'');
    bw.append(card);addZh(bw);col.append(bw);return true}
  return false;
}
...
function renderThread(){const t=$('thread');t.innerHTML='';if(!selMode){$('composer').style.visibility=''}if(typeof updateAiReplyBar==='function')updateAiReplyBar();if(!hasRole()){hideReplyBtn();return}ensureGreeting();const c=chat();
  ...
  c.forEach((m,idx)=>{
    ...
    if(m.role==='user'){renderUser(m.content,idx,m.t);if(m.seen){const rd=document.createElement('div');rd.className='read-receipt';rd.textContent='已读';t.append(rd)}}else renderAI(m.content,!!m.greet,idx,m.t);
  });
...
function renderUser(text,idx,ts){
  ...
  let lastB=null;
  allParts.forEach(p=>{
    if(isSpecialPart(p)){renderSpecialPart(col,p,'user');return}
  ...
  if(!selMode){
    ...
    col.onclick=e=>{
      ...
      openMsgSheet('我的消息',[
        ...
        {label:'删除',svg:'<svg viewBox="0 0 24 24"><path d="M3 6h18M8 6V4h8v2m-9 0v14a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V6"/></svg>',danger:true,fn:()=>{if(!confirm('删除这条消息？'))return;chat().splice(idx,1);save();renderThread()}},
      ]);
    };
  }
...
function renderAI(raw,isGreet,idx,ts){
  ...
  let lastB=null;
  parts.forEach(p=>{
    if(isSpecialPart(p)){renderSpecialPart(col,p,'ai');return}
  ...
  if(!selMode){
    ...
    col.onclick=e=>{
      ...
      actions.push({label:'删除',svg:'<svg viewBox="0 0 24 24"><path d="M3 6h18M8 6V4h8v2m-9 0v14a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V6"/></svg>',danger:true,fn:()=>{if(!confirm('删除这条消息？'))return;chat().splice(idx,1);save();renderThread()}});
...
async function send(){const text=$('input').value.trim();if(!text)return;if(!hasRole()){toast('请先新建一个角色',true);openDrawer();return}const a=curApi();if(!a.apiKey){toast('请先在设置里填 API Key',true);openSettings();return}
...
async function runAI(){
  ...
  try{
    ...
    if(isNoReply(reply)){
      ...
      if(!stripCheck(cleaned)){toast('回复为空（可能被模型拒绝或截断），可点重新生成或加大「最大回复长度」/检查破限',true);}
      pushAIReply(cleaned);
...
async function callClaude(){const a=curApi();const res=await robustFetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':a.apiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},body:JSON.stringify({model:a.model,max_tokens:(S.maxTokens||4096),system:buildSystem(),messages:buildMessages()})},120000);return await res.json()}
async function callGemini(){const a=curApi();const url=`https://generativelanguage.googleapis.com/v1beta/models/${a.model}:generateContent?key=${a.apiKey}`;const contents=buildMessages().map(m=>({role:m.role==='assistant'?'model':'user',parts:[{text:m.content}]}));const res=await robustFetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({system_instruction:{parts:[{text:buildSystem()}]},contents,generationConfig:{maxOutputTokens:(S.maxTokens||4096)}})},120000);return await res.json()}
async function callOpenAICompat(){const a=curApi();let base=a.provider==='deepseek'?'https://api.deepseek.com/v1':(a.baseUrl||'').replace(/\/$/,'');if(!base)throw new Error('请填写 Base URL');const msgs=[{role:'system',content:buildSystem()},...buildMessages()];const res=await robustFetch(base+'/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+a.apiKey},body:JSON.stringify({model:a.model,messages:msgs,max_tokens:(S.maxTokens||4096)})},120000);return await res.json()}
...
async function arGenOne(dir){
  const r=curRole(),u=curUser();
  const sys='你在帮「'+(u.userName||'我')+'」构思要发给「'+(r.roleName||'对方')+'」的下一条消息。请站在「'+(u.userName||'我')+'」的第一人称视角，写得简短、口语、像真人发手机消息。不要任何解释、引号、旁白或方括号标签，只输出可以直接发出去的回复正文。';
  const user='【最近对话】\n'+arRecentContext()+'\n\n【方向】请按「'+(dir.name||'回复')+'」'+(dir.guide?('（'+dir.guide+'）'):'')+'写一条我可以直接发出去的回复。只输出回复内容本身。';
  const out=await simpleCall(sys,user,300);
...
async function genAllAiReplies(){
  const ar=S.aiReply||{};const dirs=(ar.dirs||AR_DEF_DIRS).slice(0,ar.count||2);
  arSuggest=dirs.map(d=>({name:d.name,guide:d.guide,text:'',loading:true}));
  renderAiRepSheet();
  ...
  for(let i=0;i<dirs.length;i++){
    ...
    if(i<dirs.length-1)await new Promise(res=>setTimeout(res,600));
  }
}
...
function doTapTap(){
  ...
  // 让用户自定义拍什么
  const act=prompt('拍一拍（自定义动作，留空为默认）：','拍了拍'+(r.roleName||'TA'));
  if(act===null)return;
  const action=act.trim()||('拍了拍'+(r.roleName||'TA'));
  // 动画 topbar 头像
  ...
  // 拍一拍提示条
  const av=$('taptapTip');av.textContent=(u.userName||'你')+' '+action;av.classList.add('show');setTimeout(()=>av.classList.remove('show'),2200);
  // 作为拍一拍系统消息存入（居中显示）
  chat().push({role:'user',content:'\u0003PAT:'+action+'\u0003',t:Date.now()});
  save();renderThread();
  if(S.chatOpt.autoReply!==false)runAI();else showReplyBtn();
}
...
function refreshKeepStat(){const el=$('keepStat');if(!el)return;if(silentAudio){const st=silentAudio.ac&&silentAudio.ac.state;el.className='stat-dot on';el.textContent='后台保活：已启动（音频上下文 '+(st||'running')+'）'}else{el.className='stat-dot';el.textContent='后台保活：未启动'}}
$('btnTestKeep').onclick=()=>{startKeepAlive();if(silentAudio&&silentAudio.ac&&silentAudio.ac.state==='suspended'){silentAudio.ac.resume().then(refreshKeepStat)}setTimeout(refreshKeepStat,200);refreshKeepStat();if(silentAudio)toast('已尝试启动保活，看下方状态灯')};
