...
function freshState(){return{
  ...
  chatOpt:{split:true,splitInject:'depth',splitDepth:0,trans:false,transInject:'depth',transDepth:0,autoReply:true,showTime:true,fontSize:15,typingDelay:false,typingDelaySec:3,readNoReply:false,charStickers:true,charKaomoji:true,autoStatus:true,timeMode:'real',timeDriftMin:2,storyStart:'',patOn:true},
...
function load(){const raw=DB.get('vp6');if(raw){try{S=JSON.parse(raw)}catch(e){S=freshState()}}else S=freshState();const f=freshState();for(const k in f)if(S[k]==null)S[k]=f[k];if(!S.voiceMsg)S.voiceMsg=f.voiceMsg;if(!S.mind)S.mind=f.mind;if(S.stickers&&S.stickers.inject==null){S.stickers.inject='depth';S.stickers.depth=0}
  ...
  if(S.chatOpt){if(S.chatOpt.timeDriftMin==null)S.chatOpt.timeDriftMin=2;if(S.chatOpt.storyStart==null)S.chatOpt.storyStart='';if(S.chatOpt.patOn==null)S.chatOpt.patOn=true;}
...
function pullSettings(){
  ...
  S.chatOpt.timeDriftMin=+($('timeDrift').value)||0;S.chatOpt.storyStart=$('storyStart').value||'';S.chatOpt.patOn=$('patOn').checked;
...
// 正则编辑弹窗
let rxEditIdx=-1;
function openRxEdit(i){rxEditIdx=i;const r=curRegex().rules[i];$('rxName').value=r.name||'';$('rxGroup').value=r.group||'';$('rxFind').value=r.find||'';$('rxReplace').value=r.replace||'';$('rxTarget').value=r.target||'both';$('rxScope').value=r.applyScope||(r._isNew?'all':'chat');$('rxOn').checked=r.on!==false;$('rxEditScrim').classList.add('show');$('rxEditModal').classList.add('show')}
...
// ===== 时间管理 =====
function syncTimeUI(){
  const story=(S.chatOpt.timeMode||'real')==='fiction';
  $('storyTimeWrap').style.display=story?'block':'none';
  $('storyStart').value=S.chatOpt.storyStart||'';
  const d=S.chatOpt.timeDriftMin!=null?S.chatOpt.timeDriftMin:2;
  $('timeDrift').value=d;$('driftVal').textContent=d+' 分钟/轮';
  refreshStoryClock();
}
...
$('timeModeOn').addEventListener('change',()=>{S.chatOpt.timeMode=$('timeModeOn').checked?'real':'fiction';save();syncTimeUI()});
$('timeDrift').addEventListener('input',()=>{S.chatOpt.timeDriftMin=+$('timeDrift').value||0;$('driftVal').textContent=S.chatOpt.timeDriftMin+' 分钟/轮';save()});
$('storyStart').addEventListener('change',()=>{S.chatOpt.storyStart=$('storyStart').value||'';const c=curConvo();if(c)c.clock=null;save();refreshStoryClock()});
$('storyClockReset').addEventListener('click',()=>{const c=curConvo();if(c){c.clock=storyStartMs();save();refreshStoryClock();toast('已重置故事时间')}});
...
function storyStartMs(){const v=((S.chatOpt&&S.chatOpt.storyStart)||'').trim();if(v){const t=Date.parse(v.replace(/-/g,'/'));if(!isNaN(t))return t}return Date.now()}
...
function collectInjections(){
  ...
  if(S.mind && S.mind.on){
    ...
    if(md.genTime){
      gen.push('时间：直接照抄系统在【当前时间 · 权威】里给你的时间，一个字都不要改、不要自己推算或累加。'+(((S.chatOpt&&S.chatOpt.timeMode)==='fiction')?'如本轮剧情有明显时间跳跃，另起一行用「时间推进：+N单位」表示，不要直接改时间数字。':''));
    }
    if(gen.length){
      let text;
  ...
  // ── 时间引擎（统一：无论真实/故事，AI 都只读一个「当前时间」，绝不自行推算）──
  {
    const story=(S.chatOpt&&S.chatOpt.timeMode)==='fiction';
    const nowMs=story?storyEpoch():Date.now();
    const dstr=fmtStoryTime(nowMs);
    let block='【当前时间 · 权威】现在是 '+dstr+'。\n这是系统记录并维护的唯一权威时间，本轮请【直接采用】，绝对不要自己重新推算、累加或改写任何时间数字（年/月/日/星期/时/分都不许改）。请结合这个时间自然对话（问候、作息、白天黑夜、节日等）。';
    if(story){
      block+='\n\n【时间如何前进】时间由系统自动管理：每轮对话默认会自然流逝一小段。仅当本轮剧情出现【明显的时间跳跃】时（例如「过了三天」「一个月后」「第二天早上」「睡了一觉醒来」「下班后」），才在 <mind> 里【单独占一行】输出一条推进指令：\n时间推进：+N单位\n（单位只能是 分钟/小时/天/周/月/年，例如「时间推进：+3天」「时间推进：+8小时」）。系统会据此把权威时间往前推，并在下一轮把更新后的时间发给你。没有明显跳跃时，【不要】输出这一行，更不要在正文里直接说具体的新日期数字。';
    }
    put('depth',0,block,1);
  }
...
async function arGenOne(dir){
  const r=curRole(),u=curUser();
  const sys='你在帮「'+(u.userName||'我')+'」构思要发给「'+(r.roleName||'对方')+'」的下一条消息。请站在「'+(u.userName||'我')+'」的第一人称视角，写得简短、口语、像真人发手机消息。绝对不要任何解释、引号、旁白、心声、<标签>或方括号标签，必须一次写完整、禁止空白，只输出一句可以直接发出去的回复正文。';
  const user='【最近对话】\n'+arRecentContext()+'\n\n【方向】请按「'+(dir.name||'回复')+'」'+(dir.guide?('（'+dir.guide+'）'):'')+'写一条我可以直接发出去的回复。只输出回复内容本身，不要截断。';
  // 最多尝试 3 次，直到拿到非空正文（应对空回/被截断）
  let out='';
  for(let attempt=0;attempt<3;attempt++){
    let raw='';
    try{raw=await simpleCall(sys,user,360);}catch(e){if(attempt===2)throw e;await new Promise(res=>setTimeout(res,500));continue;}
    out=arClean(raw);
    if(out)break;
    await new Promise(res=>setTimeout(res,400));
  }
  return out||'（没想出来，点右上角🔄重试）';
}
let arSuggest=[];
