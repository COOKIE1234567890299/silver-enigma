...
function freshState(){return{
  ...
  chatOpt:{split:true,splitInject:'depth',splitDepth:0,trans:false,transInject:'depth',transDepth:0,autoReply:true,showTime:true,fontSize:15,typingDelay:false,typingDelaySec:3,readNoReply:false,charStickers:true,charKaomoji:true,autoStatus:true,timeMode:'real',timeDriftMin:2,storyStart:'',patOn:true,narrateOn:false,actionModeOn:true,callOn:true,callVoiceApi:false,callRejectChance:15},
...
function load(){const raw=DB.get('vp6');if(raw){try{S=JSON.parse(raw)}catch(e){S=freshState()}}else S=freshState();const f=freshState();for(const k in f)if(S[k]==null)S[k]=f[k];if(!S.voiceMsg)S.voiceMsg=f.voiceMsg;if(!S.mind)S.mind=f.mind;if(S.stickers&&S.stickers.inject==null){S.stickers.inject='depth';S.stickers.depth=0}
  ...
  if(S.chatOpt){if(S.chatOpt.timeDriftMin==null)S.chatOpt.timeDriftMin=2;if(S.chatOpt.storyStart==null)S.chatOpt.storyStart='';if(S.chatOpt.patOn==null)S.chatOpt.patOn=true;if(S.chatOpt.narrateOn==null)S.chatOpt.narrateOn=false;if(S.chatOpt.actionModeOn==null)S.chatOpt.actionModeOn=true;if(S.chatOpt.callOn==null)S.chatOpt.callOn=true;if(S.chatOpt.callVoiceApi==null)S.chatOpt.callVoiceApi=false;if(S.chatOpt.callRejectChance==null)S.chatOpt.callRejectChance=15;}
...
// === 通话系统 ===
let _call = {active:false,type:'voice',t0:0,tmr:null,ok:false,useApi:false};
function startCall(type) {
  ...
  const r = curRole();
  _call = {active:true,type,t0:0,tmr:null,ok:false,useApi:!!S.chatOpt.callVoiceApi};
  const av = $('callAv');
...
async function endCall() {
  if (!_call.active) return;
  const {ok, t0, type, useApi} = _call;
  _call.active = false; clearInterval(_call.tmr);
  $('callModal').classList.remove('show');
  if (!ok) { toast('对方未接听'); return; }
  const dur = t0 ? Math.floor((Date.now()-t0)/1000) : 0;
  const ds = dur > 0 ? dur+'秒' : '片刻';
  const ct = type==='video' ? '视频' : '语音';
  let desc = '';
  if (!useApi) { desc = prompt('通话中说了什么？（AI 会据此续话）','') || ''; }
  const note = ct+'通话结束，时长 '+ds+'。'+(desc ? '\n通话概要：'+desc : '');
  chat().push({role:'user', content:note, t:Date.now()});
  save(); renderThread();
  if (S.chatOpt.autoReply !== false) runAI(); else showReplyBtn();
  toast('通话结束（'+ds+'）');
}
$('callEnd')&&($('callEnd').onclick=endCall);
$('puVoiceCall')&&($('puVoiceCall').onclick=()=>startCall('voice'));
$('puVideoCall')&&($('puVideoCall').onclick=()=>startCall('video'));
...
function syncTimeUI(){
  ...
  refreshStoryClock();
  // 确保 timeModeOn 开关状态和实际 timeMode 一致
  if($('timeModeOn'))$('timeModeOn').checked=(S.chatOpt.timeMode||'real')==='real';
}
...
$('timeModeOn').addEventListener('change',()=>{
  S.chatOpt.timeMode=$('timeModeOn').checked?'real':'fiction';
  // 切换到故事时间时，若还没有 clock，初始化
  if(S.chatOpt.timeMode==='fiction'){const c=curConvo();if(c&&c.clock==null)c.clock=storyStartMs();}
  save();syncTimeUI();
  toast(S.chatOpt.timeMode==='real'?'已切换到现实时间':'已切换到故事虚拟时间，AI 将使用下方设定的时间');
});
...
function collectInjectionsOnce(){
  ...
  const key=(chat().length)+'|'+(curRole()&&curRole().id||'')+'|'+(S.chatOpt.timeMode||'real');
...
function buildMessages(){
  ...
  const msgs=_win.map(m=>{
    if(m.recalled){const _w=m.role==='user'?(curUser().userName||'你'):(curRole()&&curRole().roleName||'对方');return{role:m.role==='user'?'user':'assistant',content:'（系统：'+_w+'撤回了一条消息，你知道发生了撤回但不知道内容）'}}
...
function renderSpecialPart(col,p,side,msgIdx){
  ...
  if(patM){
    ...
    const div=document.createElement('div');div.className='sys-tip pat-tip';
  ...
  if((v=grab('transfer'))!=null){const segs=v.split('|');const amt=(segs[0]||'').trim();const note=(segs[1]||'').trim();const r=curRole(),u=curUser();const toName=side==='user'?(r?r.roleName||'对方':'对方'):(u.userName||'我');
    ...
    let footHtml='';
    if(!alreadyHandled&&side==='user'){
      // 对方发来的转账 → 可以收款或拒收
      footHtml='<div class="wt-actions"><button class="wt-btn accept">收款</button><button class="wt-btn reject">拒收</button></div>';
    }else if(!alreadyHandled&&side==='ai'){
      // 我发出去的转账 → 等对方收款
      footHtml='<div class="wt-foot"><span>等待对方收款</span><span>›</span></div>';
    }else{
    ...
    if(!alreadyHandled){
      ...
      if(acceptBtn)acceptBtn.onclick=()=>{
        const msg={role:'user',content:'（我已收款 ¥'+amt+'）',t:Date.now(),_transferAmt:amt,_transferAction:'收款'};
        chat().push(msg);save();renderThread();if(S.chatOpt.autoReply!==false)runAI();else showReplyBtn();
      };
      if(rejectBtn)rejectBtn.onclick=()=>{
        const msg={role:'user',content:'（我拒收了这笔 ¥'+amt+' 的转账）',t:Date.now(),_transferAmt:amt,_transferAction:'拒收'};
        chat().push(msg);save();renderThread();if(S.chatOpt.autoReply!==false)runAI();else showReplyBtn();
      };
    }
...
$('puNarrate')&&($('puNarrate').onclick=()=>{
  if(!hasRole()){toast('先选个角色',true);return}
  closePlus();
  const txt=prompt('旁白内容（会以居中旁白形式显示，不触发 AI 回复）：','');
...
// 以角色身份发送（AI 扮演角色主动说一句）
$('puRoleMode')&&($('puRoleMode').onclick=()=>{
  if(!hasRole()){toast('先选个角色',true);return}
  closePlus();
  const r=curRole();
  const hint=prompt('提示 AI 以「'+r.roleName+'」的身份主动说什么（留空=自由发挥）：','');
  if(hint===null)return;
  const note=hint.trim()?'（系统指令，请遵守，不要回复本段）请以「'+r.roleName+'」的身份主动发一条消息，内容方向：'+hint.trim():'（系统指令，请遵守，不要回复本段）请以「'+r.roleName+'」的身份主动发一条符合人设的消息，自由发挥。';
  chat().push({role:'user',content:note,t:Date.now(),hidden:true});
  save();renderThread();runAI();
});
